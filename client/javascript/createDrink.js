/**********
 ...
 BUGS:
 - Sometimes fucks up max value on remove(?)     fixedd
 - Switching glass gives missalignment (Should disable controls when in other menu) fixedd
...
$(document).ready(function() {
    ...
    function submitInputs(){
        ...
        for(var o = 0; o < ingredientArray.length; o++) {
            console.log('in')
            if(name == ingredientArray[o].name) {
                console.log('match name!');
                color = ingredientArray[o].color;
            }
        }
    ...
    function getTitle(){
        var newTitle = $('#inputTitle input').val();
    ...
    (function() {
        ...
        var holder = {};
          ...
          var glassHeader = new fabric.IText('Glass:', {
        ...
        var glassInfo = new fabric.IText('Cocktail', {
            fontFamily: 'Short Stack',
            top: 80,
            left: 100,
            fontSize: 15,
        ...
        var glassVolume = new fabric.IText('Volume: 15cl', {
            fontFamily: 'Short Stack',
            top: 100,
            left: 65,
            fontSize: 11,
        ...
        function switchGlass(direction) {
            ...
            fillPerCl = (glasses[drink.glassType].capacity / glasses[drink.glassType].capacityCL);
            startY = glasses[drink.glassType].rectY;
            ...
            var newVolume = 'Volume: ' + glasses[drink.glassType].capacityCL + 'cl';
            glassVolume.setText(newVolume);
            glassInfo.setText(drink.glassType);
        ...
        function editTitle(newTitle){
            ...
            if (newTitle.length > 20){
                ...
                if (/\S/.test(newTitle)){
                    ...
                    newTitle = newTitle.substring(0,(newTitle.length/2)) + '-\n' + newTitle.substring((newTitle.length/2), (newTitle.length));
                }
                title.setTop(10);
                ...
                title.setTop(20);
                title.setFontSize(40 - newTitle.length);
                title.setText(newTitle);
            }
        ...
        loadMasks();
        loadSprites();
        canvas.add(glassHeader);
        canvas.add(glassInfo);
        canvas.add(glassVolume);
        canvas.add(title);
        ...
        var titleMenu = $('titletext');
        titleMenu.onclick = function() {
            addTitleMenu();
        }

        var cancelTitle = $('cancelTitle');
    ...
    // Adding submit functionality and connection to server/databse

    $('#submitDrink').submit(function(event) {
        ...
        while(drink.ingredients.length != 0) {
            var tempIng = drink.ingredients.shift();
            ingredients.push({
                name: tempIng[0],
                size: parseFloat(tempIng[2]),
                color: tempIng[1]
            });
        }


        var glassType = drink.glassType;
        var likes = 0;
        var comments = [];
        ...
        clientSocket.emit('created_drink', JSON.stringify(recipe));
    });

    clientSocket.on('insertdrink_succesful', function() {
        alert('inserted drink!');
        window.open('./index');
    });

    clientSocket.emit('req_all_ingredients_only');
...
/*
    ...
    function testfunc() {
        ...
        for(var i = 0; i < ingredientArray.length; i++) {
            ingrArrayFormat.push(ingredientArray[i].name);
        }
        $(function() {
            console.log(ingrArrayFormat)
            $( "#tags" ).autocomplete({
                source: ingrArrayFormat
            });
        });
    }
